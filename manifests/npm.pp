...
define am_nodejs::npm (
  $target,
  $ensure            = 'present',
  $cmd_exe_path      = $am_nodejs::cmd_exe_path,
  $install_options   = [],
  $npm_path          = $am_nodejs::npm_path,
  $package           = $title,
  $source            = 'registry',
  $uninstall_options = [],
  $user              = undef,
  ...
  if $ensure == 'absent' {
    ...
    Am_nodejs::Npm::Global_config_entry<| title == 'https-proxy' |> -> Exec["npm_install_${name}"]
    Am_nodejs::Npm::Global_config_entry<| title == 'proxy' |> -> Exec["npm_install_${name}"]
